        ...
        try {
            ...
            while (true) {
                ...
                if (fromClient.equalsIgnoreCase("get-cookie")) {
                    ...
                    CookieFile cf = new CookieFile();
                    dos.writeUTF(cf.GetRandomCookieFromFile(fileName));
                    dos.flush();
                } else {
                    // Send a msg, "Invalid command from client"
    ...
    public static void StartMultiThreaderServer(int port, String fileName) {
        ...
        try {
            ...
            while (true) {
                ...
                DataInputStream dis = new DataInputStream(new BufferedInputStream(socket.getInputStream()));
                DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(socket.getOutputStream()));
                Thread tsh = new ThreadSocketHandler(socket, dis, dos);
                tsh.start();
            }
        } catch (IOException e) {
            e.printStackTrace();
